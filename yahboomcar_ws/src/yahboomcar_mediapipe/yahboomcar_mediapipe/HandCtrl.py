...
class PoseCtrlArm(Node):
    ...
    def hand_threading(self, lmList):
        if self.event.is_set():
            ...
            # Check named gestures FIRST (higher priority, more specific)
            if gesture == "Rock_on":  # Move backward
                self.media_ros.pub_vel(-0.3, 0.0, 0.0)
                sleep(1.5)
            
            elif gesture == "Thumb_up":  # Strafe left
                self.media_ros.pub_vel(0.0, 0.3, 0.0)
                sleep(1.5)
                
            elif gesture == "Shaka":  # Strafe right
                self.media_ros.pub_vel(0.0, -0.3, 0.0)
                sleep(1.5)
            ...
            elif gesture == "Three":  # Move forward and rotate right
                self.media_ros.pub_vel(0.3, 0.0, 0.3)
                sleep(1.5)
                
            elif gesture == "Four":  # Move forward and rotate left
                self.media_ros.pub_vel(0.3, 0.0, -0.3)
                sleep(1.5)
            
            # Then check generic finger-count patterns
            elif sum(fingers) == 5: 
                self.media_ros.pub_vel(0.3, 0.0,0.0)
                sleep(1.5)
